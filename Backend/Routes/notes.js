...
router.post("/:noteId/undo", verifySession, async (req, res) => {
  ...
  try {
    ...
    console.log("Version:", versions.map(v => v.id))
    console.log("Current VersionID:", note.currentVersionId)
    ...
    console.log("versions.editedBy", versions.editedBy)
    ...
    if (!isTa && !isCurrentUserAuthor) {
      ...
      res.status(403).json({ error: "You cannot undo this note because someone already edited this note" });
      return;
    }
...
router.post("/:noteId/redo", verifySession, async (req, res) => {
  const userId = req.session.userId;
  const noteId = parseInt(req.params.noteId);
  console.log("redo noteId", noteId)
  console.log("redo userId", userId)
  try {
    const note = await prisma.stickyNotes.findUnique({
      where: { id: noteId },
      select: {
    ...
    const versions = note.versions;
    const index = versions.findIndex(v => v.id === note.currentVersionId);
    if (index === -1 || index >= note.versions.length - 1) {
    ...
    console.log("userCourse", userCourse)
    const isTa = userCourse?.role === "TA";
    const isCurrentUserAuthor = nextVersion.editedBy === userId;
    console.log("isCurrentUserAuthor", isCurrentUserAuthor)
    console.log("isTa", isTa)
    if (!isTa && !isCurrentUserAuthor) {
      res.status(403).json({ error: "You cannot undo this note because someone already edited this note" });
    ...
    // redundant, the same as isCurrentUserAuthor
    // if (nextVersion.editedBy !== userId ) {
    //   res.status(403).json({ error: "You can only undo ur own edits" });
    //   return;
    // }
    await prisma.stickyNotes.update({
